...
| --- | --- | --- |
| Onboarding | Offered once on the Week tab's first visit while the profile isn't configured; also **Set Up Autopilot** (**Run Setup Again** once set up) in the Week menu (⋯) and Preferences (`AutopilotOnboardingView`, `AutopilotOnboardingSteps`) | A welcome with **Set Up Later**, then **three** one-screen questions with a progress bar and a "2 of 3" count: **What do you like?** (a grid of cuisine tiles, each a real catalog photo — tap to like, again for "no thanks", again to clear, or long-press to pick), **Anything to avoid?** (allergen and diet chips plus one "Add an ingredient" field), and **How do your weeks look?** (dinners a week, which nights, servings). At most one subtitle line per screen; longer explanations sit behind an info button. **Skip** is on every step and keeps that section's server defaults. **Finish** saves every section with one `PUT` and offers **Plan My Week**, plus a quiet **Fine-tune Autopilot** row. The cook-time mix, equipment, weekday rules, novelty, and pairings are no longer asked here — they live in Preferences (#330). |
