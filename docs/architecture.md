...
| 172 | Onboarding edits a local copy of the loaded profile, one step per section (taste, restrictions, schedule, cook-time mix, equipment, weekday rules, novelty). **Skip** restores that section from the loaded profile, which is the API's defaults before setup. **Finish** validates locally, then sends one `PUT` with all seven sections and explicit `null`s, and offers **Plan My Week**, which generates the current week and opens the review | One attributed change instead of seven, and a skipped step keeps sensible defaults. Explicit nulls keep "household default servings" and "no limit" unambiguous. |
