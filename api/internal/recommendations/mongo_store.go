...
type proposalDoc struct {
	...
	Context       *proposalContextDoc `bson:"context,omitempty"`
	SwapCount     int                 `bson:"swapCount"`
...
type proposalContextDoc struct {
	Season    string            `bson:"season,omitempty"`
	OrderDate string            `bson:"orderDate,omitempty"`
	Holidays  []holidayDoc      `bson:"holidays,omitempty"`
	Signals   []deviceSignalDoc `bson:"signals,omitempty"`
}
...
type deviceSignalDoc struct {
	Day                string `bson:"day"`
	Busyness           string `bson:"busyness,omitempty"`
	EveningFreeMinutes int    `bson:"eveningFreeMinutes,omitempty"`
	TemperatureBand    string `bson:"temperatureBand,omitempty"`
	Precipitation      string `bson:"precipitation,omitempty"`
}

func newProposalContextDoc(c ProposalContext) *proposalContextDoc {
	if c.Season == "" && c.OrderDate == "" && len(c.Holidays) == 0 && len(c.Device.Days) == 0 {
		return nil
	}
	d := &proposalContextDoc{Season: c.Season, OrderDate: c.OrderDate}
	for _, h := range c.Holidays {
		d.Holidays = append(d.Holidays, holidayDoc(h))
	}
	for _, s := range c.Device.Days {
		d.Signals = append(d.Signals, deviceSignalDoc(s))
	}
	return d
}

func (d *proposalContextDoc) toContext() ProposalContext {
	if d == nil {
		return ProposalContext{}
	}
	c := ProposalContext{Season: d.Season, OrderDate: d.OrderDate}
	for _, h := range d.Holidays {
		c.Holidays = append(c.Holidays, Holiday(h))
	}
	for _, s := range d.Signals {
		c.Device.Days = append(c.Device.Days, DeviceDaySignals(s))
	}
	return c
}
...
func newProposalDoc(p Proposal) (proposalDoc, error) {
	...
	d := proposalDoc{
		...
		SwapCount: p.SwapCount, ExcludedSlots: p.ExcludedSlots, Context: newProposalContextDoc(p.Context),
...
func (d proposalDoc) toProposal() Proposal {
	p := Proposal{
		...
		Objective: Objective(d.Objective), SwapCount: d.SwapCount, ExcludedSlots: nilIfEmpty(d.ExcludedSlots), Context: d.Context.toContext(),
...
// BusyWeeks implements Store.
func (s *MongoStore) BusyWeeks(ctx context.Context, householdID, from, to string) ([]string, error) {
	hid, err := mongodb.ParseID(householdID)
	if err != nil {
		return nil, nil
	}
	cur, err := s.contexts.Find(ctx,
		bson.D{{Key: "householdId", Value: hid}, {Key: "week", Value: bson.D{{Key: "$gte", Value: from}, {Key: "$lte", Value: to}}}, {Key: "busy", Value: true}},
		options.Find().SetProjection(bson.D{{Key: "week", Value: 1}, {Key: "_id", Value: 0}}).SetSort(bson.D{{Key: "week", Value: 1}}))
	if err != nil {
